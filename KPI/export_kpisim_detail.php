...
// Ambil ID user dari session (export untuk diri sendiri / simulasi)
$id_sim = isset($_GET['id']) ? intval($_GET['id']) : intval($_SESSION['id_user']);
...
// ================================================================
// Fungsi hitung nilai KPI Simulasi
// ================================================================
function getnilai_sim($conn, $id)
{
    $zboth = 0;
    $zbotw = 0;
    $totalws = 0;
    ...
    // Hitung total WHAT
    $stmt = $conn->prepare("SELECT * FROM tbsim_kpi WHERE id_user = ?");
    $stmt->bind_param("i", $id);
    $stmt->execute();
    $result = $stmt->get_result();

    while ($hasils = $result->fetch_assoc()) {
        $stmt2 = $conn->prepare("SELECT SUM(total) as total FROM tbsim_whats WHERE id_user = ? AND id_kpi = ?");
    ...
    $stmt3 = $conn->prepare("SELECT bobotwhat as bw FROM tbsim_bobotkpi WHERE id_user = ?");
    ...
    while ($hasilfg = $result->fetch_assoc()) {
        $stmt4 = $conn->prepare("SELECT SUM(total) as totalh FROM tbsim_hows WHERE id_user = ? AND id_kpi = ?");
    ...
    $stmt5 = $conn->prepare("SELECT bobothow as bh FROM tbsim_bobotkpi WHERE id_user = ?");
...
// Set nama file
$filename = 'Simulasi_KPI_' . str_replace(' ', '_', $data_user['nama_lngkp']) . '_' . date('Ymd_His') . '.xls';
...
// Hitung nilai KPI
$nilai_kpi = getnilai_sim($conn, $id_sim);
$rating_kpi = getkpi_sim($nilai_kpi);
...
// Ambil bobot What dan How (simulasi)
$stmt_bobot = $conn->prepare("SELECT bobotwhat, bobothow FROM tbsim_bobotkpi WHERE id_user = ?");
$stmt_bobot->bind_param("i", $id_sim);
$stmt_bobot->execute();
...
<body>
    ...
    <table border="0">
        <tr>
            <td colspan="12" class="title">SIMULASI KPI KARYAWAN</td>
        </tr>
        <tr>
            <td colspan="12" style="border:none; text-align:center;">
                <span class="sim-badge">★ DATA SIMULASI — Bukan Data KPI Resmi ★</span>
            </td>
        </tr>
        <tr>
        ...
        <tr>
            ...
            <td colspan="11" class="header-info">: <?= htmlspecialchars($data_user['nama_lngkp']); ?></td>
        </tr>
        <tr>
            <td class="header-info">NIK</td>
            <td colspan="11" class="header-info">: <?= htmlspecialchars($data_user['nik']); ?></td>
        </tr>
        <tr>
            <td class="header-info">Jabatan</td>
            <td colspan="11" class="header-info">: <?= htmlspecialchars($data_user['jabatan']); ?></td>
        </tr>
        <tr>
            <td class="header-info">Bagian</td>
            <td colspan="11" class="header-info">: <?= htmlspecialchars($data_user['bagian']); ?></td>
        </tr>
        <tr>
            <td class="header-info">Departemen</td>
            <td colspan="11" class="header-info">: <?= htmlspecialchars($data_user['departement']); ?></td>
        </tr>
        <tr>
            <td class="header-info">Atasan</td>
            <td colspan="11" class="header-info">: <?= htmlspecialchars($data_user['atasan']); ?></td>
        </tr>
        <tr>
    ...
    <!-- Ringkasan Nilai KPI Simulasi -->
    <table>
        <tr>
            <th colspan="12" class="center">RINGKASAN NILAI SIMULASI KPI</th>
        </tr>
        <tr>
            <td colspan="3" class="center" style="font-weight:bold;">Total Nilai KPI Simulasi</td>
    ...
    <?php
    // Ambil semua KPI simulasi
    $stmt_kpi = $conn->prepare("SELECT * FROM tbsim_kpi WHERE id_user = ?");
    $stmt_kpi->bind_param("i", $id_sim);
    $stmt_kpi->execute();
    ...
    while ($kpi = $result_kpi->fetch_assoc()) {
        ...
        // Ambil data What simulasi
        $stmt_what = $conn->prepare("SELECT * FROM tbsim_whats WHERE id_user = ? AND id_kpi = ?");
        $stmt_what->bind_param("ii", $id_sim, $id_kpi);
        $stmt_what->execute();
        ...
        // Ambil data How simulasi
        $stmt_how = $conn->prepare("SELECT * FROM tbsim_hows WHERE id_user = ? AND id_kpi = ?");
        $stmt_how->bind_param("ii", $id_sim, $id_kpi);
        $stmt_how->execute();
        ...
        echo "<tr>";
        echo "<th colspan='12' class='center' style='background-color:#34495e; font-size:14px;'>KPI Simulasi #$kpi_number</th>";
        echo "</tr>";
        echo "<tr>";
        ...
        for ($i = 0; $i < $max_rows; $i++) {
            ...
            if (isset($whats[$i])) {
                ...
                echo "<td>$p_what";
                // Tampilkan tipe dan target jika What B
                if ($what['tipe_what'] == 'B' && $what['target_omset'] > 0) {
                    echo "<br><small style='color:#666;'>Target: " . number_format($what['target_omset'], 2) . "</small>";
                }
                echo "</td>";
                echo "<td class='center'>$bobot_w_item%</td>";
            ...
            if (isset($hows[$i])) {
                ...
                echo "<td>$p_how";
                // Tampilkan tipe dan target jika How B
                if ($how['tipe_how'] == 'B' && $how['target_omset'] > 0) {
                    echo "<br><small style='color:#666;'>Target: " . number_format($how['target_omset'], 2) . "</small>";
                }
                echo "</td>";
                echo "<td class='center'>$bobot_h_item%</td>";
    ...
    <table>
        ...
        <tr class="bg-secondary">
            <th colspan="6" class="right">TOTAL NILAI KPI SIMULASI KESELURUHAN:</th>
    ...
    <table border="0">
        ...
        <tr>
            <td class="blue"   style="border:none;">Very Good</td>
            <td style="border:none;">: 100 - 110</td>
            <td colspan="10"   style="border:none;"></td>
        </tr>
        <tr>
            <td class="green"  style="border:none;">Excellent</td>
            <td style="border:none;">: &gt; 110</td>
            <td colspan="10"   style="border:none;"></td>
        </tr>
        <tr>
            <td colspan="12" style="border:none; color:#888; font-size:10px; padding-top:10px;">
                ★ Dokumen ini adalah hasil SIMULASI dan tidak mencerminkan data KPI resmi.
            </td>
        </tr>
    </table>
